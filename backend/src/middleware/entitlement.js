...
module.exports = async function checkEntitlement(req, res, next) {
  try {
    ...
    if (!datasetId) {
      return res.status(400).json({ status: 'error', message: 'Missing datasetId' });
    }
    ...
    if (activeSub && activeSub.quotaDownloads > activeSub.usedDownloads) {
      return next();
    }
    ...
    if (req.headers['x-api-key']) {
      ...
      if (apiKey && apiKey.userId === userId && apiKey.quota > apiKey.used) {
        return next();
      }
    }
